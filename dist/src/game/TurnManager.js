...
class TurnManager {
    ...
    async processChoice(choice, context) {
        ...
        const { gameState } = this;
        // 执行选择结果
        const result = await this.narrativeEngine.executeChoice(choice, context, gameState);
        // 更新游戏状态
    ...
    updateOmega() {
        ...
        if (pressure >= config.OMEGA.EXPONENTIAL_THRESHOLD) {
            this.gameState.omega = Math.min(config.OMEGA.MAX, omega * config.OMEGA.EXPONENTIAL_BASE);
        }
        else {
