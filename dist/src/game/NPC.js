...
class NPC extends Agent_1.Agent {
    constructor(data) {
        super({
            id: data.id,
            name: data.name,
    ...
    checkUnlock(gameState) {
        ...
        // 回合条件
        if (condition.minTurn && turn < condition.minTurn)
            return false;
        // 压强条件
        if (condition.minPressure && pressure < condition.minPressure)
            return false;
        // 玩家特质条件
        ...
        if (condition.playerItem) {
            const hasItem = player.inventory.some((i) => i.tags?.includes(condition.playerItem));
            if (!hasItem)
                return false;
        }
        // 其他NPC解锁条件
    ...
    calculateBehaviorTendency(behaviorType, player, env) {
        ...
        if (behaviorWeights.existingKnot) {
            ...
            if (knot > 0)
                tendency += behaviorWeights.existingKnot * (knot / 10);
        }
        if (behaviorWeights.deepKnot) {
            const knot = this.getKnotWith(player.id);
            if (knot >= 5)
                tendency += behaviorWeights.deepKnot;
        }
        return tendency;
    }
    /**
     ...
     */
    getAura() {
        if (this.states.fear > 70)
            return '恐惧的颤抖';
        if (this.states.aggression > 70)
            return '压抑的愤怒';
        if (this.states.hunger > 70)
            return '饥饿的虚弱';
        if (this.states.injury > 50)
            return '带伤的疲惫';
        return '沉默的警惕';
    }
    /**
