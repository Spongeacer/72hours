...
class NarrativeEngine {
     ...
     * 生成叙事
     */
    async generateNarrative(gameState) {
     ...
     * 生成选择
     */
    async generateChoices(gameState) {
     ...
     * 执行选择
     */
    async executeChoice(choice, context, gameState) {
...
关联NPC: ${context.npcs.map((n) => `${n.name}(K=${n.knot})`).join(', ') || '无'}
...
请生成一段200-400字的叙事，描述当前场景和氛围。使用第二人称"你"。`;
     ...
     * 离线模式叙事
     */
    generateOfflineNarrative(gameState) {
