...
export function generatePlayerReactions(
  ...
  if (obsessionReaction) reactions.push(obsessionReaction);
  
  // 2. 基于主导特质的反应
  const traitReaction = generateTraitReaction(player, npcBehavior, context);
  if (traitReaction) reactions.push(traitReaction);
  
  // 3. 基于本能/情境的反应
  const instinctReaction = generateInstinctReaction(player, npcBehavior, context);
  if (instinctReaction) reactions.push(instinctReaction);
 ...
 * 基于执念生成反应
 */
function generateObsessionReaction(
  player: any,
  npcBehavior: NPCBehavior,
  context: Context
): PlayerReaction | null {
 ...
 * 基于主导特质生成反应
 */
function generateTraitReaction(
  player: any,
  npcBehavior: NPCBehavior,
  context: Context
): PlayerReaction | null {
  const traits = player.traits.map((t: any) => t.id);
  const npcType = npcBehavior.type;
 ...
 * 基于本能/情境生成反应
 */
function generateInstinctReaction(
  player: any,
  npcBehavior: NPCBehavior,
  context: Context
): PlayerReaction | null {
 ...
 * 生成通用反应（补充用）
 */
function generateGenericReaction(
  player: any,
  context: Context,
  index: number
): PlayerReaction {
